...
archive-encoder/
  ...
  video/               mini-app: tiered video encoding (drop folders, UI, confirms)
  images/              mini-app: image optimization (lossless-proven / gated lossy)
...
Each mini-app stands alone -- its own drop folders, entry scripts, and outputs --
while sharing one copy of the verified engine in `core/`.
...
Double-click **`video\LaunchUI.bat`** — or run `core\make_shortcut.ps1`
...
(`core\make_icon.ps1` → `assets\icon.ico`), so it's reproducible
...
`video/sources/regular/Videos9/...` and the encode lands in
`video/encoded_outputs/regular/Videos9/...`; every file staged + renamed so a
...
1. Drop videos into **`video/sources/important/`** (irreplaceable → x265 slow CRF 22) and/or
   **`video/sources/regular/`** (bulk → NVENC HEVC q30, ~5× faster). Subfolders are mirrored.
2. `.\video\run_encode.ps1` — encodes both tiers into `video/encoded_outputs/<tier>/`.
   Resumable; re-run any time. `-Parallel` runs the CPU and GPU tiers simultaneously.
   Falls back to CPU (loudly) if no NVENC. Progress: `video/encoded_outputs/manifest.csv` + `.log`.
3. `.\video\confirm_quick.ps1` — seconds per file: every source has a counterpart, frame
   counts, durations, orientation, **bit-identical audio**.
4. `.\video\confirm_deep.ps1` — the pre-delete check: full decode, VMAF gates, plus
   source-vs-encode **screenshot pairs** at 20/50/80% saved to `video/encoded_outputs/_review/`
...
1. Drop images into **`images/sources_images/important/`** (strictly lossless) or
   **`images/sources_images/regular/`** (high-quality lossy allowed).
2. `.\images\run_image_encode.ps1` — per file: PNG/BMP/TIFF → **WebP-lossless,
   ...
   **kept as-is** (they're already lossy — re-encoding only loses quality);
   ...
   Every decision + evidence lands in `images/encoded_images/images_manifest.csv`.
3. `.\images\confirm_images_quick.ps1` — coverage + dimensions, seconds.
4. `.\images\confirm_images_deep.ps1` — the pre-delete check: pixel-hash identity for
...
The phase scripts live under `core\` — they are the engine the
...
.\core\encode_batch_fast.ps1 -SourceDir 'X:\originals' -OutDir 'Z:\encoded'
...
| Script | Role |
|---|---|
| `run_encode.ps1` | Drop-folder driver: `sources/important` → x265, `sources/regular` → NVENC, mirrored into `video/encoded_outputs/`; `-Parallel` runs both tiers at once |
...
| `ui.ps1` / `video\LaunchUI.bat` | WPF front-end: tier lists with status, safe copy-in, Run Encode launcher, double-click to play |

**`core\` — the engine and power tools:**
...
.\core\smoke_test.ps1     # 41 checks: probing, all 3 encoders, mux, audio-hash, VMAF, guards
.\core\make_corpus.ps1    # synthetic S24U-like corpus (VFR ratio 0.667, -90 rotation, GPS tags)
